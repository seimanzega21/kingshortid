...
DRAMAS_QUEUE = [
    {'id': '42000019257', 'slug': 'kembali-untuk-membalas', 'genres': ['Drama', 'Balas Dendam', 'Aksi']},
]
...
def fetch_drama_details(upstream_id, slug):
    detail_url = f'https://vidrama.asia/api/dramaboxa/drama/{upstream_id}?lang=in'
    ...
    try:
        ...
        if r.ok:
            ...
            # map to standard format expected by register
            metadata = {
                'title': book_info.get('bookName', ''),
                'description': book_info.get('introduction', ''),
                'cover': book_info.get('cover', ''),
                'totalEpisodes': book_info.get('chapterCount', 0),
                'status': 'completed' if book_info.get('bookStatus') == 2 else 'ongoing'
            }
            
            # dramaboxa has no single endpoint for episodes, we just create a pseudo-list
            # based on totalEpisodes
            total_eps = metadata['totalEpisodes']
            if total_eps > 0:
                episodes = [{'episode': i} for i in range(1, total_eps + 1)]
    except Exception as e:
        log(slug, f"⚠ Error fetching metadata: {e}")
        
    return metadata, episodes

def get_episode_stream_urls(drama_id, ep_no):
    """Get 720p stream URL for an episode"""
    url = f'https://vidrama.asia/api/dramaboxa/watch?bookId={drama_id}&episode={ep_no}&lang=in&mode=stream'
...
def download_source_file(url, local_path, slug):
    try:
        ...
        if res.returncode == 0 and os.path.exists(local_path) and os.path.getsize(local_path) > 10000:
            return True
    except Exception as e:
...
def process_drama(item):
    ...
    # 4. Process episodes
    for ep in eps_list:
        ep_no = ep.get('episode') or 1
        if ep_no in done_eps:
            continue
            
        log(slug, f"▶ Episode {ep_no}/{len(eps_list)}")
        urls = get_episode_stream_urls(upstream_id, ep_no)
        stream_url = urls.get('720p')
        if not stream_url:
