# -*- coding: utf-8 -*-
"""
Ingestion script for "Bisikan Rahasia Anisa" (Dotdrama)
Download -> Transcode (720p & 540p) -> Upload to Cloudflare R2 -> Register Drama & Episodes to KingShort API
"""
import requests
import boto3
import sys
...
TEMP_DIR     = 'd:/kingshortid/temp_bisikan_rahasia_anisa'
os.makedirs(TEMP_DIR, exist_ok=True)

DRAMA_INFO = {
    "slug": "bisikan-rahasia-anisa",
    "id": "2089905030572507138",
    "lang": "id",
...
def process_drama():
    ...
    title = meta.get('title') or "Bisikan Rahasia Anisa"
    eps_list = meta.get('episodes', [])
    total_eps = len(eps_list) or meta.get('totalEpisodes', 46)
    print(f"   Drama Title: {title}")
    print(f"   Total Episodes: {total_eps}")

    drama_id = get_or_register_drama(title, meta, total_eps, d['slug'], d.get('genres', ['Drama', 'Romantis']))
    if not drama_id:
