...
TITLE       = 'Sang Dokter Jenius'
PREFIX      = 'dramas/microdrama/sang-dokter-jenius/'
...
# Pilih resolusi tertinggi — prefer original (tanpa suffix) jika lebih besar dari 540p
...
if res_files_1080:
    best_res, res_files = '1080p', res_files_1080
elif res_files_720:
    best_res, res_files = '720p', res_files_720
elif orig_files:
    avg_orig = sum(f.get('Size',0) for f in orig_files) / max(len(orig_files), 1)
    avg_540  = sum(f.get('Size',0) for f in res_files_540) / max(len(res_files_540), 1) if res_files_540 else 0
    if avg_orig > avg_540 * 1.5:
        best_res, res_files = 'original (tertinggi)', orig_files
    else:
        best_res, res_files = '540p', (res_files_540 or orig_files)
elif res_files_540:
    best_res, res_files = '540p', res_files_540
else:
    best_res, res_files = 'unknown', video_files
...
if cover:
    ...
    try:
        ...
        for d in dramas:
            if 'dokter jenius' in d.get('title','').lower():
                cover_url = d.get('cover','')
                if cover_url:
                    print(f'  [API] Cover: {cover_url}')
