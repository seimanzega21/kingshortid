...
# 1. Update Drama list (netshort uses a JSON file)
new_json_loading = "DRAMAS_TO_PROCESS = [{'id': '9515', 'slug': 'ratu-hamil-sang-bos-mafia', 'genres': ['Drama']}]"
text = re.sub(r'try:\s*with open\(\"new_netshort_dramas\.json\".*?except.*?\]', new_json_loading, text, flags=re.DOTALL)

# 2. Update fetch_drama_details
new_fetch = """def fetch_drama_details(mid):
    import requests
    import urllib3
...
text = re.sub(r'def fetch_drama_details\(mid\):.*?return metadata, episodes', new_fetch, text, flags=re.DOTALL)

# 3. Rename PIPELINE prints
text = text.replace('NETSHORT', 'FLICKREELSV2')
text = text.replace('netshort', 'flickreelsv2')
