# -*- coding: utf-8 -*-
"""
VPS-BASED QUEUE PIPELINE: Ingest multiple shortmax dramas sequentially.
...
TEMP_DIR     = '/tmp/temp_shortmax_queue'
...
def process_single_drama(d_info, r2):
    mid = d_info['id']
    slug = d_info['slug']
    genres = d_info['genres']
    
    print("\n" + "=" * 65)
    print(f"🎬 STARTING INGESTION: {slug} (ID: {mid})")
    print("=" * 65)
    
    # 1. Fetch metadata and episodes list
    meta, eps_list = fetch_drama_details(mid)
    if not meta or not eps_list:
        print(f"❌ Failed to fetch details for drama {slug}! Skipping.")
        return False
    ...
    print("\n" + "=" * 65)
    print(f"🏁 COMPLETED INGESTION FOR {slug}!")
...
def main():
    print("=" * 65)
    print("🚀 STARTING SHORTMAX QUEUE PIPELINE")
    print(f"   Dramas to process: {len(DRAMAS_TO_PROCESS)}")
    print("=" * 65)
    ...
    for d in DRAMAS_TO_PROCESS:
        try:
            process_single_drama(d, r2)
        except Exception as e:
            print(f"❌ Error processing drama {d['slug']}: {e}")
            
    duration = time.time() - start_time
    print("\n" + "=" * 65)
    print(f"🏁 ALL SHORTMAX DRAMAS PROCESSED IN {duration/3600:.2f} HOURS!")
    print("=" * 65)
