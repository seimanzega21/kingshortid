# -*- coding: utf-8 -*-
"""
Ingestion script for 10 Dotdrama titles (5 Trending + 5 Populer / Rekomendasi)
Semua telah diverifikasi: BELUM ADA di Cloudflare R2 dan BELUM ADA di database KingShort.

Trending:
1. Istri 100 Kilo Bikin CEO Tergila-Gila (75 eps)
2. Tolong, Suamiku Nempel Terus (61 eps)
3. Sang Ratu Bangkit Demi Putrinya (50 eps)
4. Memburu Sang Pangeran Vampir (50 eps)
5. Cermin Berdarah: Cinta Iblisku (40 eps)

Populer / Rekomendasi:
6. Putri Elf dalam Balutan Baju Pelayan (40 eps)
7. Salah Mobil, Benar Jodoh (90 eps)
8. Aku Mengikat Dewi, Bukan Monster! (58 eps)
9. Sang Ratu Kembali Bersama Lima Anaknya (60 eps)
10. Tahta Runtuh, Tiga Alpha Berlutut (40 eps)
"""
import requests
import boto3
import sys
...
TEMP_DIR     = 'd:/kingshortid/temp_dotdrama_10batch'
...
DRAMAS = [
    ...
    {
        "category": "Trending",
        "title": "Istri 100 Kilo Bikin CEO Tergila-Gila",
        "slug": "istri-100-kilo-bikin-ceo-tergila-gila",
        "id": "2074397319079215106",
        "lang": "id",
        "genres": ["Romantis", "Drama"]
    },
    {
        "category": "Trending",
        "title": "Tolong, Suamiku Nempel Terus",
        "slug": "tolong-suamiku-nempel-terus",
        "id": "2054490809783476226",
        "lang": "id",
        "genres": ["Romantis", "Drama"]
    },
    {
        "category": "Trending",
        "title": "Sang Ratu Bangkit Demi Putrinya",
        "slug": "sang-ratu-bangkit-demi-putrinya",
        "id": "2086643550041374722",
        "lang": "id",
        "genres": ["Drama", "Aksi"]
    },
    {
        "category": "Trending",
        "title": "Memburu Sang Pangeran Vampir",
        "slug": "memburu-sang-pangeran-vampir",
        "id": "2089900566943858689",
        "lang": "id",
        "genres": ["Fantasi", "Romantis"]
    },
    {
        "category": "Trending",
        "title": "Cermin Berdarah: Cinta Iblisku",
        "slug": "cermin-berdarah-cinta-iblisku",
        "id": "2081567032482455553",
        "lang": "id",
        "genres": ["Romantis", "Fantasi"]
    },

    # 5 Populer / Rekomendasi
    {
        "category": "Populer/Rekomendasi",
        "title": "Putri Elf dalam Balutan Baju Pelayan",
        "slug": "putri-elf-dalam-balutan-baju-pelayan",
        "id": "2095058944734986242",
        "lang": "id",
        "genres": ["Fantasi", "Drama"]
    },
    {
        "category": "Populer/Rekomendasi",
        "title": "Salah Mobil, Benar Jodoh",
        "slug": "salah-mobil-benar-jodoh",
        "id": "2076499611926200322",
        "lang": "id",
        "genres": ["Romantis", "Komedi"]
    },
    {
        "category": "Populer/Rekomendasi",
        "title": "Aku Mengikat Dewi, Bukan Monster!",
        "slug": "aku-mengikat-dewi-bukan-monster",
        "id": "2084571292056936449",
        "lang": "id",
        "genres": ["Aksi", "Fantasi"]
    },
    {
        "category": "Populer/Rekomendasi",
        "title": "Sang Ratu Kembali Bersama Lima Anaknya",
        "slug": "sang-ratu-kembali-bersama-lima-anaknya",
        "id": "2086992703678283778",
        "lang": "id",
        "genres": ["Drama", "Keluarga"]
    },
    {
        "category": "Populer/Rekomendasi",
        "title": "Tahta Runtuh, Tiga Alpha Berlutut",
        "slug": "tahta-runtuh-tiga-alpha-berlutut",
        "id": "2083078010806198273",
        "lang": "id",
        "genres": ["Romantis", "Fantasi"]
    }
]
...
def process_drama(r2, d):
    print("\n" + "=" * 65, flush=True)
    print(f"🎬 STARTING INGESTION [{d['category']}]: {d['title']} ({d['slug']})", flush=True)
    print(f"   ID: {d['id']}", flush=True)
    print("=" * 65, flush=True)
...
def main():
    print("=" * 65, flush=True)
    print(f"STARTING BATCH INGESTION FOR {len(DRAMAS)} DOTDRAMA TITLES", flush=True)
    print(f"5 Trending + 5 Populer/Rekomendasi (Semua Fresh, Belum Ada di R2/DB)", flush=True)
    print("=" * 65, flush=True)
    r2 = get_r2()
    for idx, d in enumerate(DRAMAS, 1):
        print(f"\n[{idx}/{len(DRAMAS)}] Processing drama: {d['title']} ({d['slug']})...", flush=True)
        try:
            process_drama(r2, d)
        except Exception as e:
            print(f"❌ Error processing drama {d['slug']}: {e}", flush=True)
    print("\n🎉 ALL 10 DRAMAS HAVE BEEN PROCESSED!", flush=True)
