# -*- coding: utf-8 -*-
"""
VPS-BASED QUEUE PIPELINE: Ingest multiple stardusttv dramas sequentially.
Bypasses local machine network limits and processes all episodes automatically.
"""
import requests
import boto3
import sys
...
TEMP_DIR     = '/tmp/temp_queue'
os.makedirs(TEMP_DIR, exist_ok=True)

# List of dramas to ingest sequentially
DRAMAS_TO_PROCESS = [
    {'id': '18657', 'slug': 'maaf-aku-hanya-dewa-pedang', 'genres': ['Action', 'Wuxia', 'Drama']},
    {'id': '18364', 'slug': 'mantan-kaya-terus-minta-balikan', 'genres': ['Romantis', 'Drama']},
    {'id': '19712', 'slug': 'cinta-dan-peluru-semua-untuknya', 'genres': ['Action', 'Romantis', 'Drama']},
    {'id': '16855', 'slug': 'dia-istri-sang-taipan', 'genres': ['Romantis', 'Drama']},
    {'id': '17871', 'slug': 'demi-anak-aku-bangkit-dari-kematian', 'genres': ['Drama', 'Family']},
    {'id': '19613', 'slug': 'ceo-jatuh-cinta-pada-ibu-6-anak', 'genres': ['Romantis', 'Komedi', 'Family']},
    {'id': '18734', 'slug': 'sudah-bilang-aku-ibu-miliarder', 'genres': ['Drama', 'Family']},
    {'id': '18783', 'slug': 'rebut-kembali-takhta-dewa', 'genres': ['Action', 'Fantasy', 'Wuxia']},
    {'id': '19208', 'slug': 'sang-penagih-utang-takdir', 'genres': ['Drama', 'Thriller']},
    {'id': '19570', 'slug': 'salah-jurus-tapi-aku-jadi-terkuat', 'genres': ['Action', 'Fantasy', 'Komedi']},
    {'id': '8619',  'slug': 'mata-emas-dari-jurang', 'genres': ['Action', 'Thriller', 'Fantasy']}
]
...
def get_or_register_drama(metadata, slug, genres):
    ...
    # Check if already exists in admin panel
    ...
    payload = {
        ...
        'cover': cover_r2,
        'genres': genres,
        'totalEpisodes': metadata.get('chapterCount') or 60,
        'status': 'ongoing',
        'country': 'China',
        'language': 'Indonesia',
...
def process_drama(d_info, r2):
    ...
    print("\n" + "=" * 65)
    print(f"🎬 PROCESSING DRAMA: {slug} (ID: {mid})")
    print("=" * 65)
    
    # 1. Fetch metadata and stream links
    meta, eps_list = fetch_drama_details(mid)
    if not meta or not eps_list:
        print(f"❌ Failed to fetch details for {slug}! Skipping.")
        return False
    ...
    drama_id = get_or_register_drama(meta, slug, genres)
    if not drama_id:
        print(f"❌ Failed to register drama {slug}! Skipping.")
        return False
...
def main():
    print("=" * 65)
    print("🚀 STARTING STARDUSTTV QUEUE INGESTION PIPELINE ON VPS")
    print("=" * 65)
    ...
    for d in DRAMAS_TO_PROCESS:
        try:
            ...
            print(f"\n❌ Uncaught error processing drama {d['slug']}: {e}")
            
    elapsed = time.time() - total_start
    print("\n" + "=" * 65)
    print(f"🏁 ALL DRAMAS IN QUEUE PROCESSED!")
