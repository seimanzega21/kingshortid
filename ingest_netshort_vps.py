# -*- coding: utf-8 -*-
"""
VPS-BASED PIPELINE: Membela Masakan Tradisional (ID: 2072567284573876225)
Provider: netshort
Now with automatic subtitle download & registration!
"""
import requests
import boto3
import sys
...
# ─── CONFIG ────────────────────────────────────────────────────────────────
DRAMA_ID_WEB = '2072567284573876225'
DRAMA_SLUG   = 'membela-masakan-tradisional'
GENRES       = ['Drama', 'Romansa', 'Kuliner']
...
def fetch_episode_url(mid, ep_no):
    ...
    hdrs = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36',
        'Referer': f'https://vidrama.asia/movie/membela-masakan-tradisional--{mid}?provider=netshort&lang=in',
    }
    for attempt in range(1, 6):
        try:
...
def get_or_register_drama(metadata, total_eps):
...
def process_subtitles(r2, ep_id, ep_no, subtitles_list):
    ...
    for sub in subtitles_list:
        ...
        r2_key = f"dramas/{DRAMA_SLUG}/ep{ep_no:03d}_{db_lang}.vtt"
...
def main():
    print("=" * 65)
    print(f"🎬 PROCESSING NETSHORT DRAMA: {DRAMA_SLUG} (ID: {DRAMA_ID_WEB})")
    print("=" * 65)
    ...
    drama_id = get_or_register_drama(meta, total_eps)
    if not drama_id:
    ...
    for ep in eps_list:
        ...
        mp4_url, subs_list = fetch_episode_url(DRAMA_ID_WEB, ep_no)
        if not mp4_url:
        ...
        if ep_no in done_eps:
            ...
            process_subtitles(r2, ep_id, ep_no, subs_list)
            success_count += 1
            continue
