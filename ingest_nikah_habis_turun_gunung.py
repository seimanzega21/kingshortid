# -*- coding: utf-8 -*-
"""
VPS-BASED PIPELINE: (Dub)Nikah Habis Turun Gunung
Provider: melolov3
"""
import requests
import boto3
...
# ─── CONFIG ────────────────────────────────────────────────────────────────
DRAMA_ID_WEB = '7657771952576154629'
DRAMA_SLUG   = 'nikah-habis-turun-gunung'
GENRES       = ['Aksi', 'Drama', 'Romantis']
...
TEMP_DIR     = '/tmp/temp_nikah_gunung'
...
def fetch_drama_details():
    detail_url = f'https://vidrama.asia/api/melolov3/series?id={DRAMA_ID_WEB}&lang=id'
    videos_url = f'https://vidrama.asia/api/melolov3/multi-video?id={DRAMA_ID_WEB}&lang=id'
...
def upload_cover_to_r2(r2, cover_url):
    if not cover_url:
        return ""
    local_jpg = os.path.join(TEMP_DIR, f"{DRAMA_SLUG}_cover_hq.jpg")
    try:
        ...
        # Upload
        r2_key = f"dramas/covers/{DRAMA_SLUG}_cover_hq.jpg"
...
def get_or_register_drama(r2, metadata):
    title = metadata.get('title') or '(Dub)Nikah Habis Turun Gunung'
    ...
    cover_r2 = upload_cover_to_r2(r2, cover_raw)
    ...
    payload = {
        ...
        'cover': cover_r2,
        'genres': GENRES,
        'totalEpisodes': metadata.get('episode_count') or 83,
        'status': 'ongoing',
        'country': 'China',
        'language': 'Indonesia',
...
def main():
    print("=================================================================")
    print("MELOLOV3 INGESTION PIPELINE: (Dub)Nikah Habis Turun Gunung")
    print("=================================================================")
    
    r2 = get_r2()
    
    # 1. Fetch metadata and video list
    meta, eps_list = fetch_drama_details()
    if not meta or not eps_list:
        print("❌ Failed to fetch metadata or episodes list. Aborting.")
        return
    ...
    drama_db_id = get_or_register_drama(r2, meta)
    if not drama_db_id:
        print("❌ Failed to get/register drama in DB. Aborting.")
        return
    ...
    for ep in eps_list:
        ...
        local_720, local_540 = transcode_to_resolutions(local_raw, ep_no)
        ...
        # Upload
        r2_key_720 = f"dramas/netshort/{DRAMA_SLUG}/ep{ep_no:03d}.mp4"
        r2_key_540 = f"dramas/netshort/{DRAMA_SLUG}/ep{ep_no:03d}_540p.mp4"
