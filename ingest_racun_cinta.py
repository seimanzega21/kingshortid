# -*- coding: utf-8 -*-
"""
Ingestion script for Racun Cinta Sang Mafia (dramabox: 42000026080)
"""
import requests
import boto3
import json
...
DRAMA_CONFIG = {
    "slug": "racun-cinta-sang-mafia",
    "id": "42000026080",
    "lang": "in",
    "genres": ["Drama", "Romantis", "Eksklusif"]
}
...
def download_and_transcode(video_url, ep_no, slug):
    ...
    # Download source directly via requests
    for attempt in range(1, 4):
        try:
            resp = requests.get(video_url, headers=HDR, stream=True, timeout=60)
            if resp.ok:
                with open(src, 'wb') as f:
                    for chunk in resp.iter_content(chunk_size=1024*1024):
                        if chunk:
                            f.write(chunk)
                if os.path.exists(src) and os.path.getsize(src) > 500*1024:
                    break
            print(f"      ⚠ DL attempt {attempt} failed, status: {resp.status_code}")
        except Exception as e:
    ...
    for attempt in range(1, 4):
        res = subprocess.run(
            ...
            capture_output=True, text=True, errors='ignore', timeout=240
        )
...
def finalize_drama(drama_id, total_eps):
    print(f"   🎉 Finalizing drama {drama_id} (isActive=True, status=completed)...")
    payload = {
        'isActive': True,
        'status': 'completed',
        'totalEpisodes': total_eps
    }
    r = requests.patch(f"{API_BASE}/api/dramas/{drama_id}", headers=ADMIN_HDR, json=payload, timeout=20)
    if r.ok:
        print(f"   ✓ Drama activated and marked as completed!")
    else:
        print(f"   ⚠ Failed to finalize drama: {r.status_code} {r.text[:100]}")

def main():
    print("=" * 65)
    print("STARTING DRAMABOX INGESTION: Racun Cinta Sang Mafia")
    print("=" * 65)
    r2 = get_r2()
    d = DRAMA_CONFIG
    ...
    drama_id = get_or_register_drama(r2, title, meta, total_eps, d['slug'], d.get('genres', ['Drama']))
    if not drama_id:
        return

    # Check already registered episodes
    done_eps = {}
    ...
    print("\n" + "=" * 65)
    print(f"🏁 INGESTION RESULT FOR {title}!")
    print(f"   Success: {success_count}/{total_eps} | Failed: {fail_count}/{total_eps}")
    print("=" * 65)

    if success_count == total_eps:
        finalize_drama(drama_id, total_eps)
