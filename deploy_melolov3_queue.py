...
def main():
    print("Uploading ingest_melolov3_queue_vps.py to VPS...")
    try:
        ...
        sftp = ssh.open_sftp()
        sftp.put('d:/kingshortid/ingest_melolov3_queue_vps.py', '/tmp/ingest_melolov3_queue_vps.py')
        sftp.close()
        ...
        ssh.exec_command("pkill -f ingest_melolov3_queue_vps.py")
        
        # Launch process in background (reusing log file path)
        print("Launching ingest_melolov3_queue_vps.py on VPS in background...")
        ssh.exec_command("touch /var/log/ingest_nikah_habis_turun_gunung.log && chmod 666 /var/log/ingest_nikah_habis_turun_gunung.log")
        ssh.exec_command("nohup python3 -u /tmp/ingest_melolov3_queue_vps.py > /var/log/ingest_nikah_habis_turun_gunung.log 2>&1 &")
