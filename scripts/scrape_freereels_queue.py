...
# -*- coding: utf-8 -*-
"""
KingShort Vidrama Scraper for FreeReels Provider - Queue Processor
===================================================================
Reads scripts/freereels_queue.json, processes the next pending drama,
...
QUEUE_PATH = Path(__file__).parent / 'freereels_queue.json'
TEMP_DIR = Path(tempfile.gettempdir()) / 'freereels_queue_scraper'
...
def api_get_or_create_drama(detail, slug, cover_url):
    ...
    payload = {
        ...
        'cover': cover_url,
        'genres': genres,
        'totalEpisodes': detail.get('chapterCount') or detail.get('chapters') or 70,
...
def scrape_drama(r2, vid_id, test_mode=False):
    ...
    url = f"https://vidrama.asia/api/freereels?action=detail&id={vid_id}"
    try:
        ...
        if not data_dict.get('success'):
            ...
            return False
        detail = data_dict.get('data', {})
        chapters = detail.get('episodesList') or detail.get('list') or []
    except Exception as e:
    ...
    for ch in chapters:
        ep_no = ch.get('episodeNo') or ch.get('index')
        if ep_no is None:
            continue
        ...
        vurl = ch.get('videoPath')
        subtitles = ch.get('subtitleList', [])
        ...
        for s in subtitles:
            ...
            if lang in ['id', 'id-id', 'id_id', 'in', 'in-id', 'in_id', 'indonesia']:
