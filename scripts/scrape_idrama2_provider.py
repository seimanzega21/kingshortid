...
# -*- coding: utf-8 -*-
"""
KingShort Vidrama Scraper for idrama2 Provider (Unencrypted)
==================================================
Downloads dramas from melolov2 provider on vidrama.asia,
...
def scrape_single_drama(r2, movie_id, is_test_run=False):
    ...
    try:
        ...
        if db_id:
            ...
            db_id = api_get_or_create_drama(detail_remapped, slug, r2_cover_url)
            if not db_id:
            ...
            if not r2_exists(r2, cover_key):
                try:
                    cover_src = detail.get('cover_url') or detail.get('compress_cover_url')
                    if cover_src:
        ...
        for ep in eps:
            ...
            # Stream URL and subtitles
            vurl = None
            for p in (ep.get('play_info_list') or []):
            ...
            subtitles = (ep.get('subtitle_list') or []) + (ep.get('screentext_list') or [])
            ...
            for attempt in range(3):
                try:
                    ...
                    if res.returncode == 0 and raw_path.exists() and raw_path.stat().st_size > 50*1024:
                        ...
                        print(f" [Attempt {attempt+1}] Download failed or file too small. Retrying...")
                        time.sleep(5)
                except Exception as e:
