...
for p in range(1, 100):
    try:
        r = requests.get(f'https://vidrama.asia/api/netshortv2/feed/{p}?lang=id_ID', headers=WEB_HDRS, verify=False, timeout=10)
        items = r.json().get('data', [])
        if not items:
            break
        for it in items:
            title = it.get('title', '').lower()
            if 'ikatan cantik' in title or 'abadi' in title:
                found = it
                print(f"Found on page {p}: {it}")
        if found:
            break
