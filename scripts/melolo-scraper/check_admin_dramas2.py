...
try:
    r = requests.get("https://api.shortlovers.id/api/dramas?includeInactive=true&limit=50")
    data = r.json()
    ...
    pending = [d for d in dramas if d.get("isActive") is False] # Only false, not undefined
