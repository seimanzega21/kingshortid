...
# -*- coding: utf-8 -*-
"""
KingShort Scraper - Desainer Top Balik ke Era 95 (Netshort V2)
...
TARGET_ID   = '2096782583337111554'
TARGET_SLUG = 'desainer-top-balik-ke-era-95'
...
def main():
    r2 = get_r2()
    slug = TARGET_SLUG
    vidrama_id = TARGET_ID
    prefix = f"netshortv2/{slug}"

    print("="*60)
    print(f"🎬 SEDOT DRAMA: Netshort V2 ({vidrama_id})")
    print("="*60)

    # 1. Ambil detail metadata dari Vidrama API
    detail_url = f"{VIDRAMA_API}/detail/{vidrama_id}?lang=id_ID"
    print(f"[*] Mengambil info drama dari: {detail_url}")
    ...
    if not detail_r.ok or detail_r.json().get('code') != 200:
        print(f"[ERROR] Gagal mengambil detail drama. Status: {detail_r.status_code}")
        print("Response:", detail_r.text[:300])
        return

    detail = detail_r.json()['data']
    title = detail.get('title', 'Desainer Top Balik ke Era 95')
    total_eps = detail.get('totalEpisodes', 0)

    print(f"[+] Judul: {title}")
    print(f"[+] Total Episode: {total_eps}")
    print(f"[+] Cover: {detail.get('cover')}")
    print(f"[+] Status: {'Tamat' if detail.get('isFinished') else 'Ongoing'}")

    local_save_dir = Path("D:/Video Drama/Facebook") / title.replace(":", " ").replace("/", " ")
    local_save_dir.mkdir(parents=True, exist_ok=True)
    print(f"[+] Folder Lokal: {local_save_dir}")

    # 2. Registrasi / Cek Database
    cover_key = f"{prefix}/cover.webp"
    r2_cover_url = f"{R2_PUBLIC}/{cover_key}"

    db_id = check_duplicate_in_db(title)
    if db_id:
        print(f"[INFO] Judul '{title}' sudah ada di DB (ID: {db_id}). Melanjutkan proses upload episode...")
    else:
        db_id = api_get_or_create_drama(detail, slug, r2_cover_url)
        if not db_id:
            print("[ERROR] Gagal mendaftarkan drama ke database KingShort.")
            return
        print(f"[DB] Sukses didaftarkan ke Database KingShort (ID: {db_id}, Status: Pending)")

    # 3. Upload Cover ke R2
    if not r2_exists(r2, cover_key):
        try:
            print("[*] Mengunduh & upload cover ke R2...")
            ...
            print(f"[WARN] Gagal upload cover: {e}")
    else:
        print("[COVER] Cover sudah ada di R2")

    # 4. Loop Episodes
    print(f"\n[*] Mulai proses {total_eps} episode...")
    ...
    for ep_no in range(1, total_eps + 1):
        ...
        if r2_exists(r2, k720):
            ...
            if not local_file.exists():
                try:
                    print(f"   [INFO] Unduh ep{ep_no:03d} dari R2 ke lokal...", end="", flush=True)
                    ...
                    print(f" [WARN] Gagal simpan lokal: {e}")
            success += 1
            continue
        ...
        if not vurls:
            print(f" [WARN] URL video tidak ditemukan!")
            skipped += 1
            continue
        ...
        try:
            ...
            if encode_720_and_540(raw, o720, o540):
                ...
                print(" [ERROR] Gagal encoding ffmpeg!")
                failed += 1
    ...
    print("\n" + "="*60)
    print(f"🎉 SELESAI PROSES DRAMA: {title}")
    print(f"  Total Episode: {total_eps}")
