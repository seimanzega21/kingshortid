...
SEARCH_TERMS = [
    ('berhenti berjudi', 'Berhenti Berjudi, Utamakan Keluarga'),
    ('pengemis', 'Pengemis Itu Sangat Berkuasa'),
    ('berkuasa', 'Pengemis Itu Sangat Berkuasa'),
]

print('=== MENCARI DI VIDRAMA ===')
    ...
    try:
        ...
        if r.ok:
            ...
            for it in items:
                ...
                for keywords, expected in SEARCH_TERMS:
                    if keywords in title:
                        print(f"MATCH: {it.get('title')} | ID: {it.get('id')} | Eps: {it.get('totalEpisodes')}")
    except: pass

print()
print('=== DETAIL CEK UNTUK YANG SUDAH DITEMUKAN ===')
# Check #1 Berhenti Berjudi
for vid_id, expected_title in [('unknown', 'Berhenti Berjudi')]:
    pass
