...
print('=== SEARCH VIDRAMA BY EXACT TITLE ===')
results = []
    ...
    try:
        ...
        if r.ok:
            ...
            for it in items:
                ...
                results.append((it.get('title'), it.get('id'), it.get('totalEpisodes')))
                # Check all relevant keywords
                if 'pengemis' in title:
                    print(f"PENGEMIS: {it.get('title')} | ID: {it.get('id')} | Eps: {it.get('totalEpisodes')}")
                if 'berjudi' in title:
                    print(f"BERJUDI: {it.get('title')} | ID: {it.get('id')} | Eps: {it.get('totalEpisodes')}")
                if 'berhenti' in title:
                    print(f"BERHENTI: {it.get('title')} | ID: {it.get('id')} | Eps: {it.get('totalEpisodes')}")
                if 'romantis' in title and 'dingin' in title:
                    print(f"ROMANTIS DINGIN: {it.get('title')} | ID: {it.get('id')} | Eps: {it.get('totalEpisodes')}")
    except Exception as e:
        print(f'Error page {page}: {e}')
        break

print(f"\nTotal scanned: {len(results)} dramas")
