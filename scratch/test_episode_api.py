...
for ep in [29, 30, 31, 32]:
    for lang in ['id_ID', 'in', 'en']:
        url = f'https://vidrama.asia/api/dramabox3/episode/{BOOK_ID}/{ep}?lang={lang}'
        print(f"\nFetching EP {ep} with lang {lang}...")
        ...
        if r.ok:
            ...
            if code == 200:
                ...
                if subtitles:
                    print("  Subtitles detail:", json.dumps(subtitles, indent=2))
                for v in videos:
                    print(f"    - Quality: {v.get('quality')}, URL: {v.get('url')[:80]}...")
            else:
