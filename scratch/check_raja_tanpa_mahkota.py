...
print("=== FETCHING DRAMA METADATA ===")
url_meta = f"https://vidrama.asia/api/dramabox3/drama/{BOOK_ID}?lang=in"
r = requests.get(url_meta, headers=VIDRAMA_HDR, verify=False)
if r.ok:
    data = r.json().get('data', {})
    print("Book Name:", data.get('bookName'))
    print("Chapter Count (Total EP):", data.get('chapterCount'))
    print("Introduction:", data.get('introduction'))
    print("Cover URL:", data.get('cover'))
    print("Book Status:", data.get('bookStatus'))
else:
    print(f"Failed to fetch metadata: {r.status_code} - {r.text}")

# 2. Fetch ep 1 watch details
print("\n=== FETCHING EP 1 DETAILS ===")
url_watch = f"https://vidrama.asia/api/dramabox3/watch?bookId={BOOK_ID}&episode=1&lang=in"
r_watch = requests.get(url_watch, headers=VIDRAMA_HDR, verify=False)
if r_watch.ok:
    wdata = r_watch.json()
    print("Watch API success:", wdata.get('success'))
    print("Subtitles count:", len(wdata.get('subtitles', [])))
    print("Subtitles list:", json.dumps(wdata.get('subtitles', []), indent=2))
    print("Qualities:")
    for q in wdata.get('availableQualities', []):
        print(f"  - {q.get('label')}: {q.get('url')[:100]}...")
else:
    print(f"Failed to fetch watch details: {r_watch.status_code} - {r_watch.text}")
