...
def list_sub_prefixes(parent_prefix):
    paginator = r2.get_paginator('list_objects_v2')
    prefixes = []
    for page in paginator.paginate(Bucket=BUCKET, Delimiter='/', Prefix=parent_prefix):
...
def main():
    ...
    # 2. Fetch ALL R2 prefixes, including nested folders
    roots = ['dramas/', 'netshortv2/', 'microdrama/', 'melolo/', 'goodshort/', 'freereels/']
    all_r2_prefixes = []
    
    print("[*] Fetching all prefixes in R2 (with nesting)...")
    for root in roots:
        # direct sub-folders
        direct = list_sub_prefixes(root)
        for d in direct:
            # If it's a nested parent folder like dramas/microdrama/ or dramas/shortmax/, traverse one level deeper
            if d in ['dramas/microdrama/', 'dramas/shortmax/', 'dramas/covers/']:
                nested = list_sub_prefixes(d)
                all_r2_prefixes.extend(nested)
                print(f"    -> Traversed nested {d}: {len(nested)} prefixes")
            else:
                all_r2_prefixes.append(d)
        print(f"    -> {len(direct)} direct prefixes in {root}")
    ...
    for target in TARGET_DRAMAS:
        ...
        # Filter matching duplicate folder names across roots
        best_prefix = None
        if matched_prefixes:
            # Sort: prefer netshortv2/ and dramas/microdrama/ and dramas/ over others
            def rank(prefix):
                if 'netshortv2/' in prefix:
                    return 0
                if 'dramas/microdrama/' in prefix:
                    return 1
                if 'dramas/' in prefix:
                    return 2
                return 3
            matched_prefixes.sort(key=rank)
    ...
    print("\n=== HASIL PENCARIAN & PEMETAAN (REKURSif) ===")
