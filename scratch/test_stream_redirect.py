...
VIDRAMA_HDR = {
    ...
    'cookie': COOKIE,
    'priority': 'u=1, i',
    'referer': f'https://vidrama.asia/watch/mendengar-isi-hatimu--{BOOK_ID}/30?provider=dramabox3&lang=in',
    'user-agent': 'Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Mobile Safari/537.36'
}

for ep in [29, 30, 31, 32]:
    print(f"\n--- Episode {ep} ---")
    url = f"https://dramabox.iniurl.net/api/stream?bookId={BOOK_ID}&episode={ep}&quality=720&lang=in"
    r = requests.get(url, headers=VIDRAMA_HDR, allow_redirects=False, verify=False)
