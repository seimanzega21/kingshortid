...
def main():
    url = 'https://vidrama.asia/api/netshortv2/episode/2012092639088226305/1?lang=id_ID'
    ...
    if r.ok:
        ...
        for k, v in data.items():
            if k != 'videos':
                ...
                print(f"videos count: {len(v)}")
                for item in v:
                    print(f"  - Quality: {item.get('quality')} | URL: {item.get('url')[:80]}...")
    else:
