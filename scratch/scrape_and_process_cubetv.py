...
of "Gila untuk Mencintai" drama from cubetv provider.
"""
import os
import sys
import re
...
TEMP_DIR = WORKSPACE_DIR / 'temp_cubetv'
TEMP_DIR.mkdir(exist_ok=True)
LOG_FILE = WORKSPACE_DIR / 'scratch' / 'scrape_cubetv.log'
...
def get_registered_episodes(drama_db_id):
    ...
    if r.ok:
        ...
        # Build map: episodeNumber -> (episodeId, videoUrl, videoUrl540p)
...
def merge_episodes_hardsub(dest_folder, ep_files, ep_vtt_files, start, end):
    ...
    try:
        # Run FFmpeg (silence output unless there is an error)
...
def main():
    ...
    with open(LOG_FILE, 'w', encoding='utf-8') as f:
        f.write("--- CUBETV SCRAPER PIPELINE LOG STARTED ---\n")

    log("=" * 65)
    log("STARTING CUBETV INGESTION AND FACEBOOK PROCESSING PIPELINE")
    log("=" * 65)

    r2 = get_r2()

    # 1. Fetch metadata & episode list
    log(f"Fetching metadata for upstream ID: {UPSTREAM_ID}...")
    ...
    try:
        ...
        if not r_det.ok or not r_eps.ok:
            log(f"[ERROR] Failed to fetch metadata or episodes from upstream API. Statuses: {r_det.status_code}, {r_eps.ok}")
            sys.exit(1)
    except Exception as e:
    ...
    # Episode files dictionaries for Facebook processing
    ...
    for ep in eps_list:
        ...
        if ep_no in registered_eps:
            ...
            if not local_vtt_path.exists() or not local_srt_path.exists():
                ...
                try:
                    ...
                    # Fallback to scraping it from upstream directly if R2 fails
                    if sub_url:
        ...
        try:
            ...
            if sub_url:
                ...
                try:
                    ...
                    if sub_r.ok:
                        ...
                        # Convert to VTT and save locally
                        vtt_text = srt_to_vtt(sub_r.text)
                        with open(local_vtt_path, 'w', encoding='utf-8') as f:
                            f.write(vtt_text)
            ...
            if ep_db_id:
                ...
                # Copy transcoded 720p video file to final destination locally
                import shutil
    ...
    log("\n" + "=" * 65)
    log("ALL EPISODES INGESTED. STARTING FACEBOOK MERGE PACKAGES (PER 5 EPISODES)...")
    log("=" * 65)
