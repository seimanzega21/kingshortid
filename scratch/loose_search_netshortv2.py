...
queries = ['Ditinggal Nikah', 'Dikejar Harta', 'Akhiri Sandiwara', 'Mulai Cinta', 'Ditinggal', 'Akhiri']

def main():
    print("Starting loose searches on netshortv2...")
    for q in queries:
        url = f"https://vidrama.asia/api/netshortv2/search/{requests.utils.quote(q)}/1?lang=id_ID"
        try:
            ...
            if r.ok:
                ...
                for res in results[:5]:
                    print(f"   - {res.get('title')} (ID: {res.get('id')})")
            else:
