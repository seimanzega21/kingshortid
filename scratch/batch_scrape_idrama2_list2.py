# -*- coding: utf-8 -*-
"""
Batch Scraper and Ingestion Script for 10 Dramas (List 2)
Provider: idrama2
"""
import requests
import boto3
...
TEMP_DIR = WORKSPACE_DIR / 'temp_batch_list2'
TEMP_DIR.mkdir(exist_ok=True)
LOG_FILE = WORKSPACE_DIR / 'scratch' / 'batch_scrape_list2.log'
...
def get_db_drama_by_title(title):
    """Find drama DB ID by title (exact match or slug match)"""
...
def register_drama_api(title, meta, cover_url):
    """Register drama in DB via API"""
    payload = {
        'title': title,
        'description': meta.get('introduction', '').strip(),
        'cover': cover_url,
        'genres': [],
...
def get_registered_episodes(drama_db_id):
    """Get list of episode numbers already registered in DB"""
    ...
    if r.ok:
        ...
        return {e.get('episodeNumber') for e in ep_list if e.get('episodeNumber')}
    return set()

def fetch_episode_unlock_info(upstream_id, ep_no):
    """Call unlock endpoint to get stream URL and subtitle info"""
    url = f"https://vidrama.asia/api/idrama2/unlock/{upstream_id}/{ep_no}?lang=id"
    r = requests.get(url, headers=HEADERS, verify=False, timeout=20)
    if r.ok:
        return r.json().get('target_ep_info', {})
    return {}
...
def main():
    parser = argparse.ArgumentParser(description="Batch Scrape 10 dramas from idrama2 - List 2")
    ...
    log("============================================================")
    log("BATCH SCRAPING PIPELINE STARTED (LIST 2)")
    log("============================================================")

    # List of Upstream IDs provided by user
    drama_ids = [
        '160000641975',  # Aku Adalah Putri Dari Dunia Terlarang
        '160000642209',  # Tidur Sekejap Terbangun Sepuluh Tahun Kemudian
        '160000641510',  # Sistem Kultivasi Pengubah Takdir
        '160000641812',  # Ratu Mafia Yang Menjinakkan Hati Pria Desa
        '160000641563',  # Dewa Yang Turun Dari Gunung
        '161001641450',  # Bunga Violet Di Balik Pakaian
        '161001641615',  # Dua Bayi Kecil Satukan Ayah Ibu
        '161001641010',  # Putri Sejati Sang Mahaguru
        '161001641786',  # Dari Istri Diam-Diam Jadi Pembalasan Maut
        '160000641625',  # Lima Tahun Cinta Dibalas Pengkhianatan
    ]
    ...
    success_count = 0
    max_workers = 1  # Sequential scraping (one-by-one)
    log(f"Starting sequential scraping with {max_workers} worker...")
