...
print(f"Checking subtitles on upstream provider netshortv2 for ID {ns_id}...")
    ...
    try:
        ...
        if r.ok:
            ...
            if data.get('code') == 200:
                ep_d = data.get('data', {})
                subtitles = ep_d.get('subtitles', [])
                if not subtitles:
                    no_subs.append(ep_no)
                else:
                    langs = [s.get('language') or s.get('lang') for s in subtitles]
                    has_subs.append((ep_no, langs, subtitles))
            else:
                print(f"  EP {ep_no} -> API code {data.get('code')}: {data.get('message')}")
        else:
            print(f"  EP {ep_no} -> HTTP {r.status_code}")
    except Exception as e:
        print(f"  EP {ep_no} -> Error: {e}")

print(f"\nEpisodes WITH subtitles on upstream ({len(has_subs)}):")
for item in has_subs[:5]:
    print(f"  - EP {item[0]}: {item[1]}")
if len(has_subs) > 5:
    print(f"  ... and {len(has_subs) - 5} more episodes.")

print(f"\nEpisodes WITHOUT subtitles on upstream ({len(no_subs)}):", no_subs)
