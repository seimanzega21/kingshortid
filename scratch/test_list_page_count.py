...
while True:
    ...
    print(f"Found {len(dataList)} dramas. hasMore={has_more}", flush=True)
    total_dramas += len(dataList)
    if not has_more or not dataList:
        break
    page += 1
    time.sleep(0.5)

print(f"Grand Total Dramas in catalog: {total_dramas}", flush=True)
