...
langs = ['en', 'zh', 'id', 'es', 'fr', '']
for ep in [30, 31]:
    for lang in langs:
        url = f'https://vidrama.asia/api/dramabox3/watch?bookId={BOOK_ID}&episode={ep}'
        if lang:
            url += f'&lang={lang}'
        
        print(f"Fetching EP {ep} with lang '{lang}'...")
        r = requests.get(url, headers=VIDRAMA_HDR, verify=False)
        if r.ok:
            res = r.json()
            subtitles = res.get('subtitles', [])
            if subtitles:
                print(f"  FOUND Subtitles for EP {ep} (lang={lang}):")
                print(json.dumps(subtitles, indent=2))
            else:
                print(f"  No subtitles for EP {ep} (lang={lang})")
        else:
