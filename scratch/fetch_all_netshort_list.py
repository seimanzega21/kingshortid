...
if r.ok:
    ...
    if match:
        ...
        print(f"Total dramas in netshortv2 listing: {len(dramas)}")
        ...
        for d in dramas:
            ...
            if 'sinar' in title.lower() or 'mencari' in title.lower():
                ...
                found = True
        if not found:
            print("Not found via keyword search. Listing first 10:")
            for d in dramas[:10]:
                print(f"  - {d.get('title')} (ID: {d.get('id')})")
    else:
        print("Could not find __NEXT_DATA__ script block in page HTML")
else:
    print(f"HTTP GET failed: {r.status_code}")
