# -*- coding: utf-8 -*-
"""
Orchestration script for scraping, transcoding, DB registering, and Facebook processing
of the "Pilot Cilik di Ketinggian 30.000 Kaki" drama from dramawavev2 provider.
"""
import os
import sys
import re
...
TEMP_DIR = WORKSPACE_DIR / 'temp_dramawave'
TEMP_DIR.mkdir(exist_ok=True)
LOG_FILE = WORKSPACE_DIR / 'scratch' / 'scrape_dramawave.log'
...
def download_m3u8_stream(m3u8_url, local_path):
    # Set headers since some providers check referer
...
def main():
    ...
    with open(LOG_FILE, 'w', encoding='utf-8') as f:
        f.write("--- DRAMAWAVE PIPELINE LOG STARTED ---\n")

    log("=" * 70)
    log("STARTING INGESTION AND PROCESSING FOR PILOT CILIK")
    log("=" * 70)

    r2 = get_r2()

    # 1. Fetch metadata & episode list
    detail_url = f"https://vidrama.asia/api/dramawave?action=detail&id={DRAMA_UPSTREAM_ID}"
    log(f"Fetching drama metadata from: {detail_url}")

    try:
        r_det = requests.get(detail_url, headers=HEADERS, verify=False, timeout=20)
        if not r_det.ok:
            log(f"[ERROR] Failed to fetch metadata from upstream API. Status: {r_det.status_code}")
            return
    except Exception as e:
    ...
    det_json = r_det.json()
    if not det_json.get('success'):
        log(f"[ERROR] Upstream detail returned success=False. Body: {det_json}")
        return

    det_data = det_json.get('data', {})
    eps_list = det_data.get('list', [])

    title = (det_data.get('title') or det_data.get('bookName', '')).strip()
    description = (det_data.get('description') or det_data.get('introduction', '')).strip()
    cover_raw = det_data.get('cover') or det_data.get('image', '')
    total_eps = len(eps_list)

    if not title:
        log("[ERROR] Drama title is empty, exiting.")
        return
    ...
    if drama_db_id:
        ...
        if not drama_db_id:
            log("[ERROR] Drama registration failed. Exiting.")
            return
    ...
    for ep in eps_list:
        ep_no = ep.get('episodeNo') or ep.get('chapterIndex', 0) + 1
        m3u8_url = ep.get('videoPath')
        ...
        # Find Indonesian subtitle url (code is usually id-ID, in-ID, id, or in)
        sub_url = None
        for s in subs_list:
            if s.get('language') in ['id-ID', 'id', 'in', 'in-ID', 'id_ID'] and s.get('subtitle'):
                sub_url = s['subtitle']
                break
        ...
        if ep_no in registered_eps:
            ...
            # Download Indonesian subtitle VTT from R2 and convert to SRT
        ...
        if not m3u8_url:
            log("  Stream URL empty in detail, attempting fallback stream resolution with retries...")
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    r_st = requests.get(stream_api, headers=HEADERS, timeout=20, verify=False)
                    if r_st.ok:
                        res_json = r_st.json()
                        if res_json.get('success'):
                            st_data = res_json.get('data', {})
                            ...
                            log(f"    [WARN] Stream API returned success=False (Attempt {attempt}/{max_retries}). Msg: {res_json.get('msg')}")
                    else:
                        log(f"    [WARN] Stream API returned HTTP {r_st.status_code} (Attempt {attempt}/{max_retries})")
                except Exception as ex:
                    log(f"    [WARN] Stream API request error (Attempt {attempt}/{max_retries}): {ex}")
        ...
        try:
            ...
            # 4. Subtitle download, convert, and upload to R2
            r2_sub_url = None
            if sub_url:
