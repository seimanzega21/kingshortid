...
# Test 1: Standard URL
url1 = f'https://vidrama.asia/api/netshortv2/episode/{ns_id}/{ep_no}?lang=id_ID'
r1 = requests.get(url1, headers=WEB_HDRS, verify=False)
sub_url1 = r1.json().get('data', {}).get('subtitles', [])[0].get('url')
print("Sub URL 1:", sub_url1[:150] + "...")

# Test 2: URL with cache buster
t = int(time.time())
url2 = f'https://vidrama.asia/api/netshortv2/episode/{ns_id}/{ep_no}?lang=id_ID&_t={t}'
r2 = requests.get(url2, headers=WEB_HDRS, verify=False)
sub_url2 = r2.json().get('data', {}).get('subtitles', [])[0].get('url')
print("Sub URL 2 (cache buster):", sub_url2[:150] + "...")

# Check if they are different or have different signature
print("Are they identical:", sub_url1 == sub_url2)
