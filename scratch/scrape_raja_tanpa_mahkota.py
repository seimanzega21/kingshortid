# -*- coding: utf-8 -*-
"""
Scraper for "Raja Tanpa Mahkota (Sulih Suara)" - Phase 2
Book ID: 42000003547 | Drama ID: n2edv3mw0hw1i60k0ox4y7e3
- Skips already registered episodes.
- If already registered, downloads the 720p file from R2 directly to the local folder.
- For new episodes, does the full download, encode with faststart, upload to R2, DB registration, and saves the 720p locally.
- After all 76 episodes are ready, concatenates the 720p files in groups of 3 to the gabungan subfolder.
"""
import requests
import boto3
import sys
...
# ─── CONFIG ─────────────────────────────────────────────────────────────────
BOOK_ID = '42000003547'
BOOK_SLUG = 'raja-tanpa-mahkota-sulih-suara'
...
VIDRAMA_HDR = {
    ...
    'cookie': COOKIE,
    'priority': 'u=1, i',
    'referer': f'https://vidrama.asia/watch/raja-tanpa-mahkota--{BOOK_ID}/1?provider=dramabox3&lang=in',
    'user-agent': 'Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Mobile Safari/537.36'
}

# Directories
WORKSPACE_DIR = Path(__file__).resolve().parent.parent
TEMP_DIR = WORKSPACE_DIR / 'temp_raja'
TEMP_DIR.mkdir(exist_ok=True)

# User defined local path
LOCAL_SAVE_DIR = Path(r"D:\video drama\upload facebook\1. raja-tanpa-mahkota-sulih-suara")
...
def main():
    print("=" * 60)
    print("RAJA TANPA MAHKOTA PIPELINE - PHASE 2 (LOCAL SAVE & CONCAT)")
    print("=" * 60)
    ...
    registered_eps = get_registered_episodes()
    print("Registered episode numbers:", sorted(list(registered_eps)))
    
    # Loop through episodes 1 to 76
    ...
    for ep_no in range(1, TOTAL_EPS + 1):
        ...
        if ep_no in registered_eps:
            ...
            # If not present in local folder, download from R2 directly!
        ...
        if not stream_urls:
            print(f"    ✗ No stream URLs found for EP {ep_no}, skipping")
            continue
        ...
        try:
            ...
            ep_id = register_episode(DRAMA_ID, ep_no, r2_url_720, r2_url_540)
            if ep_id:
    ...
    for start in range(1, TOTAL_EPS + 1, 3):
        ...
        merged_filename = f"raja-tanpa-mahkota-ep{start:02d}-ep{end:02d}.mp4"
