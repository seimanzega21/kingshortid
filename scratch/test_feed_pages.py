...
for page in range(1, 10):
    url = f"https://vidrama.asia/api/netshortv2/feed/{page}?provider=cubetv&lang=id_ID"
    try:
        ...
        if r.status_code == 200:
            ...
            for it in items:
                d_id = it.get('id')
                title = it.get('title')
                all_dramas.append((d_id, title))
                print(f"  - ID: {d_id} | Title: {title}")
        else:
...
print(f"\nTotal dramas found for cubetv: {len(all_dramas)}")
