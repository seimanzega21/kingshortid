...
for item in mapped:
    ...
    try:
        ...
        if r.ok:
            ...
            code = data.get('code')
            d = data.get('data', {})
            cover = d.get('cover') if isinstance(d, dict) else None
            title = d.get('title') if isinstance(d, dict) else None
            print(f"ID {ns_id} | Title: {title} | Code: {code} | Cover: {cover}")
        else:
            print(f"ID {ns_id} | Failed HTTP: {r.status_code}")
    except Exception as e:
        print(f"ID {ns_id} | Error: {e}")
