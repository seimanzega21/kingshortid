...
url = f'https://vidrama.asia/movie/mendengar-isi-hatimu--{BOOK_ID}?provider=dramabox3&lang=in'
...
if r.ok:
    ...
    # Search for subtitle or script tags
    scripts = re.findall(r'<script\b[^>]*>(.*?)</script>', r.text, re.DOTALL)
    print(f"Found {len(scripts)} script tags.")
    
    # Save first 10000 chars of HTML to file for viewing
    with open('scratch/watch_page_snippet.html', 'w', encoding='utf-8') as f:
        f.write(r.text)
    print("Saved watch page to scratch/watch_page_snippet.html")
else:
