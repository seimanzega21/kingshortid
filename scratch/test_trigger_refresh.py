...
# 1. Fetch current expiry
url_api = f'https://vidrama.asia/api/netshortv2/episode/{ns_id}/{ep_no}?lang=id_ID'
r = requests.get(url_api, headers=WEB_HDRS, verify=False)
sub_url_before = r.json().get('data', {}).get('subtitles', [])[0].get('url')
print("Expiry before:", re.search(r'Expires=(\d+)', sub_url_before).group(1))

# 2. Get the watch pages with different referers and parameters
pages = [
    f'https://vidrama.asia/movie/mencari-sinar-di-lautan--{ns_id}?provider=netshortv2&lang=in',
    f'https://vidrama.asia/movie/mencari-sinar-di-lautan--{ns_id}?provider=netshortv2&lang=id',
    f'https://vidrama.asia/watch/netshortv2/{ns_id}/{ep_no}?lang=id_ID',
    f'https://vidrama.asia/watch/netshortv2/{ns_id}/{ep_no}?lang=in_ID'
]

for p in pages:
    print(f"Triggering page: {p}...")
    try:
        requests.get(p, headers=WEB_HDRS, verify=False, timeout=10)
    except:
        pass
    time.sleep(1)

# 3. Fetch again
r_after = requests.get(url_api, headers=WEB_HDRS, verify=False)
sub_url_after = r_after.json().get('data', {}).get('subtitles', [])[0].get('url')
print("Expiry after:", re.search(r'Expires=(\d+)', sub_url_after).group(1))
print("Did it refresh:", sub_url_before != sub_url_after)
