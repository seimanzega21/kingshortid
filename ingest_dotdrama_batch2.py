...
TEMP_DIR     = 'd:/kingshortid/temp_dotdrama_queue_2'
os.makedirs(TEMP_DIR, exist_ok=True)

DRAMAS = [
    {
        "slug": "ratu-hilang-raja-naga-gelisah",
        "id": "2089537245849812994",
        "lang": "id",
        "genres": ["Drama", "Fantasi"] 
    },
    {
        "slug": "gema-malam-kembalinya-sang-ratu",
        "id": "2089653087384076290",
        "lang": "id",
...
def fetch_drama_data(mid):
    ...
    print(f"   ðŸŒ Fetching details for drama ID {mid} (dotdrama REST)...")
    ...
    for attempt in range(1, 6):
        try:
            ...
            print(f"      âš  Detail Connection error: {e}")
        time.sleep(3)
    return None
...
def download_and_transcode(video_url, ep_no):
    ...
    for attempt in range(1, 4):
        try:
            ...
            if res.returncode == 0 and os.path.exists(local_source) and os.path.getsize(local_source) > 1024*1024:
                ...
                print(f"      âš  Source DL Attempt {attempt} failed: {res.stderr.strip()[-200:]}")
        except Exception as e:
            print(f"      âš  Source DL Connection error (attempt {attempt}/3): {e}")
        time.sleep(3)
    ...
    for attempt in range(1, 4):
        ...
        if res.returncode == 0 and os.path.exists(local_720) and os.path.getsize(local_720) > 1024*1024:
            ...
            print(f"      âš  720p Attempt {attempt} failed: {res.stderr.strip()[-200:]}")
    ...
    for attempt in range(1, 4):
        ...
        if res.returncode == 0 and os.path.exists(local_540) and os.path.getsize(local_540) > 500000:
            ...
            print(f"      âš  540p Attempt {attempt} failed: {res.stderr.strip()[-200:]}")
            if attempt < 3: time.sleep(3)
...
def upload_to_r2(r2, local_path, r2_key):
    try:
        ...
        print(f"      âœ“ Uploaded {os.path.basename(r2_key)} ({size_mb:.1f} MB)")
...
def get_or_register_drama(title, metadata, total_eps, slug, genres):
    ...
    if r.ok:
        ...
        for d in dramas:
            if title.lower() == d.get('title', '').lower():
                print(f"   âœ“ Drama already registered in DB: {d['id']}")
                return d['id']
    ...
    if r.ok:
        ...
        print(f"   âœ“ New drama registered! ID: {drama_id}")
        return drama_id
    else:
        print(f"   âœ— Drama registration failed: {r.status_code} {r.text[:200]}")
        return None
...
def process_drama(r2, d):
    print("\n" + "=" * 65)
    print(f"ðŸŽ¬ STARTING INGESTION: {d['slug']} (ID: {d['id']})")
    print("=" * 65)
    
    meta = fetch_drama_data(d['id'])
    if not meta:
        print(f"âŒ Failed to fetch drama details for {d['slug']}! Skipping.")
        return False
    ...
    if not drama_id:
        print(f"âŒ Failed to register drama {d['slug']}! Skipping.")
        return False
    ...
    for ep in eps_list:
        ...
        print(f"\n   ðŸ“º Episode {ep_no}/{total_eps}:")
        
        if ep_no in done_eps:
            print("      âœ“ Video already registered")
            success_count += 1
            continue
        ...
        if not video_url:
            print(f"      âŒ Failed to fetch stream URL for EP {ep_no}")
            fail_count += 1
            continue
        ...
        if not url_720:
            ...
            if not local_720:
                print(f"      âŒ Download/transcode failed for EP {ep_no}")
                fail_count += 1
                continue
            ...
            if not url_720:
                print(f"      âŒ R2 upload failed for EP {ep_no}")
                fail_count += 1
                continue
        else:
            print("      âœ“ Already in R2")
            
        ep_id = register_episode(drama_id, ep_no, url_720, url_540)
        if ep_id:
            print(f"      âœ… Video Done! ID: {ep_id}")
            success_count += 1
            done_eps[ep_no] = ep_id
        else:
            print(f"      âŒ DB Registration failed for EP {ep_no}")
            fail_count += 1
            
    print("\n" + "=" * 65)
    print(f"ðŸ INGESTION COMPLETE FOR {title}!")
...
def main():
    ...
    for d in DRAMAS:
        try:
            ...
            print(f"âŒ Error processing drama {d['slug']}: {e}")
