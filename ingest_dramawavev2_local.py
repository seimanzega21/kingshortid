# -*- coding: utf-8 -*-
"""
VPS-BASED PIPELINE: Ingestion script specifically for dramawavev2
"""
import requests
import boto3
import sys
...
TEMP_DIR     = 'd:/kingshortid/temp_dramawavev2_queue'
os.makedirs(TEMP_DIR, exist_ok=True)

DRAMAS = [
    {
        "slug": "pewaris-terbuang-adalah-penyihir-ilahi",
        "id": "RYPtQ25Q0h",
        "lang": "in",
...
def fetch_drama_details(mid):
    url = f"https://vidrama.asia/api/dramawavev2?action=detail&id={mid}&lang=in"
    print(f"   🌐 Fetching details for drama ID {mid} (dramawavev2 REST)...")
...
def fetch_episode_url(mid, ep_no):
    url = f"https://vidrama.asia/api/dramawavev2?action=stream&id={mid}&episode={ep_no}&lang=in"
    hdrs = {'User-Agent': 'Mozilla/5.0'}
    for attempt in range(1, 6):
        try:
            r = requests.get(url, headers=hdrs, timeout=20)
            if r.ok:
                ...
                if res.get('success'):
                    ...
                    if videoUrl.startswith('/api/video-proxy'):
                        parsed = urllib.parse.urlparse(videoUrl)
                    ...
                    for sub in subs:
                        s_url = sub.get('url', '')
                        if s_url.startswith('/api/video-proxy'):
                            parsed = urllib.parse.urlparse(s_url)
                            qs = urllib.parse.parse_qs(parsed.query)
                            if 'url' in qs:
                                sub['url'] = qs['url'][0]
                                
                    return videoUrl, subs
        except Exception as e:
            pass
        time.sleep(3)
    return None, []
...
def process_subtitles(r2, ep_id, ep_no, subtitles_list, slug):
    ...
    for sub in subtitles_list:
        lang = sub.get('lang') or sub.get('language') or sub.get('label') or ''
        raw_url = sub.get('url') or sub.get('src') or ''
        if not lang or not raw_url:
            continue
            
        if 'id-ID' in lang or 'Indonesian' in lang or lang == 'id':
            db_lang = 'id'
            db_label = 'Bahasa Indonesia'
            is_default = True
        elif 'en-US' in lang or 'English' in lang or lang == 'en':
            db_lang = 'en'
            db_label = 'English'
            is_default = False
        else:
            db_lang = lang.split('-')[0].lower()
            db_label = lang
            is_default = False
...
def process_drama(r2, d):
    ...
    title = meta.get('title') or "Pewaris Terbuang Adalah Penyihir Ilahi"
    total_eps = meta.get('chapterCount', 54)
...
def main():
    print("=" * 65)
    print("STARTING DRAMAWAVEV2 PIPELINE")
    print("=" * 65)
    r2 = get_r2()
    for d in DRAMAS:
        try:
    ...
    print("\nALL DRAMAWAVEV2 DRAMAS PROCESSED!")
